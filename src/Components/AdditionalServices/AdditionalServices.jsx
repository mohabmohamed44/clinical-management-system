...
export default function AdditionalServices() {
    ...
    try {
      ...
      const formattedOffers = data.map(offer => ({
        ...
        finalPrice: offer.discount_percentage 
          ? calculateDiscountedPrice(offer.original_price, offer.discount_percentage)
          : offer.price
      }));
  ...
  const nextSlide = () => {
    setCurrentIndex(prev => (prev === Math.max(0, offers.length - 3) ? 0 : prev + 1));
  };

  const prevSlide = () => {
    setCurrentIndex(prev => (prev === 0 ? Math.max(0, offers.length - 3) : prev - 1));
  };
    ...
    <div className="py-12 w-full bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 relative">
        ...
        <div className="overflow-hidden" ref={carouselRef}>
          ...
          >
            {offers.map((offer) => (
              <div key={offer.id} className="flex-none w-full sm:w-1/2 lg:w-1/3 px-3 pb-4">
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-lg text-blue-600">${offer.finalPrice}</span>
                        {offer.discount_percentage > 0 && (
                    ...
                    <div className="flex items-center mb-6">
                      ...
                      />
                      <div>
                        <p className="font-medium text-gray-900">{offer.doctor.name}</p>
                        <div className="flex items-center">
                          <Star className="w-4 h-4 text-yellow-400 mr-1" />
                          <span className="text-sm text-gray-600">{offer.doctor.rate}/5</span>
                        </div>
                      </div>
                    </div>
